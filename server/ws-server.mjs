...
class FlatlandClient {
   World = null;
	world = null; // extend socket.
	lastMessage = Date.now();
	
}

//JSOX.toJSOX( "", GameWorld )
class FlatlandProtocol extends Protocol{ 
	constructor( opts ) {
		super( opts );
		this.on("connect", function( ws) {
			ws.noDelay = true;
		} );
...
new FlatlandProtocol( { port: Number(process.argv[2]) || process.env.PORT || 5000
                      , protocol: "Flatland"
                      , resourcePath: "../flatland-client" } );
