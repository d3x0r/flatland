	...
	return {
		...
		_write(msg,complete_at_end) {
			...
			function recoverIdent(cInt) {
				_DEBUG_PARSING&&console.log( "Recover Ident char:", cInt, String.fromCodePoint(cInt), "word:", word );
			...
			function openObject() {
				...
				if( word > WORD_POS_RESET && word < WORD_POS_FIELD )
					recoverIdent( 123 /* '{' */ );
				var protoDef;
				protoDef = getProto(); // lookup classname and get protodef(if any)
				if( parse_context == CONTEXT_UNKNOWN ) {
					if( word == WORD_POS_FIELD /*|| word == WORD_POS_AFTER_FIELD*/ && (protoDef || val.string.length) ) {
						...
						if( !protoDef || !protoDef.protoDef && val.string ) // class creation is redundant...
						{
							...
							if( !cls ) {
								...
								, protoCon: (protoDef && protoDef.protoCon) || privateProto.constructor
								 , fields : [] } );
					...
					if( word != WORD_POS_RESET || val.value_type == VALUE_STRING ) {
						...
						if( protoDef = localFromProtoTypes.get( val.string ) ) {
							...
							if( !cls )
							{
								...
								_DEBUG_STRINGIFY && console.log( "Assume it's a class that we should know about but don't.", val.string );
								nextMode = CONTEXT_OBJECT_FIELD;
							}
							else
								nextMode = CONTEXT_CLASS_VALUE;
					...
					case 125/*'}'*/:
						...
						if( parse_context == CONTEXT_CLASS_FIELD ) {
							...
							if( _DEBUG_PARSING ) console.log( "close object; empty object", val, elements );
								...
								val.value_type = VALUE_OBJECT;
								if( current_proto ) {
									console.log( "SOMETHING SHOULD AHVE BEEN REPLACED HERE??" );
									console.log( "The other version only revives on init" );
									elements = new current_proto.cb( elements, undefined, undefined );
							...
							default:
								...
								*/
								...
								if( word === WORD_POS_RESET ) {
									...
									val.value_type = VALUE_STRING;
									val.string += str;
									if( _DEBUG_PARSING ) console.log( "START/CONTINUE IDENTIFER" );
									break;
